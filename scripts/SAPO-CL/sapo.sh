...
if [ $TASK_ID -eq 1 ]; then
    ...
    MODEL_NAME=./checkpoints/Qwen3-VL-2B/SAPO-CL/training/${PREV_DATASET}
fi
...
OUTPUT_DIR=./checkpoints/Qwen3-VL-2B/SAPO-CL/training/${CUR_DATASET}

mkdir -p "${OUTPUT_DIR}"

# TRL 0.27.0 SAPO uses soft gating; ordinary GRPO clipping is not used.
