...
if [ $TASK_ID -eq 1 ]; then
    ...
    MODEL_NAME=./checkpoints/Qwen3-VL-4B/DrGRPO-CL/training/${PREV_DATASET}
fi
...
OUTPUT_DIR=./checkpoints/Qwen3-VL-4B/DrGRPO-CL/training/${CUR_DATASET}

mkdir -p "${OUTPUT_DIR}"

# Dr. GRPO: normalize by max_completion_length and disable reward std scaling.
# Advantages remain group-mean centered; clipping is symmetric.
