...
if [ $TASK_ID -eq 1 ]; then
    ...
    MODEL_NAME=./checkpoints/Qwen3-VL-4B/CISPO-CL/training/${PREV_DATASET}
fi
...
OUTPUT_DIR=./checkpoints/Qwen3-VL-4B/CISPO-CL/training/${CUR_DATASET}

mkdir -p "${OUTPUT_DIR}"

# TRL 0.27.0 CISPO: epsilon_high is the direct IS-weight cap (not 1 + epsilon).
# Clipped IS weights are detached; epsilon (lower clipping) is not used.
